...
`ref` is optional, by default it will use `master`.
...
Even though this backend could be started standalone and remotely, I would not recommend doing that. Terraform does not perform any encryption before sending the state to HTTP backend, so that would not be secure. Instead, I would suggest to use this backend as a "wrapper" - start it locally on the same host you're about to run Terraform.

You should be good to Go. Just use it like that:
...
If you're still willing to accept all risks and want to make it remotely accessible, assuming you are adding other security measurements (like API gateway or service mesh), you can use this option `--address=:6061` and that way it will bind to `0.0.0.0`. You can change the port too.
...
Due to this, I couldn't make a proper native Terraform backend implementation for Git on a side, it should be implemented and added to https://github.com/hashicorp/terraform code base. There is an open ticket to do it https://github.com/hashicorp/terraform/issues/24603, but it is unclear when this would happen (if it will at all). That said I figured this HTTP backend implementation would be helpful for now.

## Why

So you must be wondering why storing Terraform state in Git might be a good idea.
...
Below is a proposal how a native Git backend implementation would look like. This HTTP backend follows it too.
